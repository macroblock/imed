...
func main() {
	...
	cmdLine.Elements(
		...
		cli.Flag("-li           : targeted integrated loudness (LUFS, default = -23.0)", &flagLI),
		cli.Flag("-lra          : max allowed loudness range (LU) or 'off' to disable LRA check", &flagLRA),
		cli.Flag("-tp           : max allowed true peaks (dBFS) or 'off' to disable TP calculation (default = -1.0)", &flagTP),
		cli.Flag("-mp           : max allowed sample peaks (dBFS)", &flagMP),
		cli.Flag("-lprec        : integrated loudness precision", &flagPrecision),
